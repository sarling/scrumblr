...
var db = function(callback) {
	console.log('Opening redis connection on URL: ' + conf.redis);
...
db.prototype = {
	...
	// Snapshot commands
	createSnapshot: function(room, timestamp, snapshot) {
		var snapshotString = JSON.stringify( snapshot);
		redisClient.zadd(REDIS_PREFIX + '-room:' + room + '-snapshots', timestamp, snapshotString,
			function (err, res) {
	...
	getSnapshot: function(room, timestamp, callback) {
		redisClient.zrangebyscore(REDIS_PREFIX + '-room:' + room + '-snapshots', timestamp, timestamp, 
			function(err, res) {
	...
	getAllSnapshots: function(room, callback) {
		redisClient.zrange(REDIS_PREFIX + '-room:' + room + '-snapshots', 0, -1, 
			function(err, res) {
				var snapshots = {};

				for (var i in res) {
					var resObject = JSON.parse(res[i]);
					snapshots[resObject.timestamp] = resObject;
				}
				callback(snapshots);
			}
		);
	...
	deleteSnapshot: function(room, timestamp) {
		redisClient.zremrangebyscore(REDIS_PREFIX + '-room:' + room + '-snapshots', timestamp, timestamp);
	},
	...
	cardEditText: function(room, id, value) {
	...
	cardEditPrio: function(room, id, value) {
		redisClient.hget(REDIS_PREFIX + '-room:' + room + '-cards', id, function(err, res) {
			var card = JSON.parse(res);
			if (card !== null) {
				card.prio = value;
	...
	cardEditDue: function(room, id, value) {
		redisClient.hget(REDIS_PREFIX + '-room:' + room + '-cards', id, function(err, res) {
			var card = JSON.parse(res);
			if (card !== null) {
				card.due = value;
	...
	cardSetProperties: function(room, id, properties) {
		redisClient.hget(REDIS_PREFIX + '-room:' + room + '-cards', id, function(err, res) {
			var card = JSON.parse(res);
			if (card !== null) {
	...
	// properties
	getAllProperties: function(room, callback) {
		redisClient.lrange(REDIS_PREFIX + '-room:' + room + '-properties', 0, -1, function(err, res) {
			var resArr = [];
			for( var i in res) {
				resArr.push( JSON.parse(res[i]));
			}
			callback(resArr);
		});
	},

	setProperties: function(room, data) {
		// delete all properties
		redisClient.del(REDIS_PREFIX + '-room:' + room + '-properties', function () {
			// add all properties back in again
			async.forEachSeries(
				properties,
				function( item, callback ) {
					redisClient.rpush(REDIS_PREFIX + '-room:' + room + '-properties', JSON.stringify(item),
						function (err, res) {
							callback();
						}
					);
	...
	getCardProperties: function(room, cardid, callback) {
		redisClient.hget(REDIS_PREFIX + '-room:' + room + '-cardproperties', cardid, function(err, res) {
			callback(JSON.parse(res));
		});
	},

	setCardProperties: function(room, cardid, properties) {
		var propertiesString = JSON.stringify( properties);
		redisClient.hset(REDIS_PREFIX + '-room:' + room + '-cardproperties', cardid, propertiesString);
	},
